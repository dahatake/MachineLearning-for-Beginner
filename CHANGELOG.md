...
- `setup/setup-windows.cmd` と `setup/setup-windows.ps1`（PowerShell 5.1 / 7 両対応）、`setup/setup-mac.sh`、`setup/setup-linux.sh`、共通処理 `setup/lib/common.sh` を追加し、Anaconda（既定）/ Miniconda / venv の3方式でセットアップできるようにしました。Anaconda / Miniconda は Python 本体も含めて導入します。venv は既存の Python（3.10〜3.14）があればそれを使い、なければ Windows では python.org の Python 3.12.10 をユーザー単位（`%LOCALAPPDATA%\Programs\Python\Python312`、管理者権限不要）で、macOS では同じ版の python.org 公式 pkg を確認とパスワード入力のうえで自動的に導入します。Linux ではディストリビューションの Python を使います。macOS/Linux の Conda 操作は、`conda-forge` / `pytorch` チャンネルのみを使う一時 `condarc` で行い、`defaults` チャンネルの利用規約未同意による非対話環境での失敗を避けます。
